...
class Appmerce_Epdq_Model_Process extends Varien_Object
{
    /**
    ...
    public function invoice($order)
    {
        ...
        if (!$order->hasInvoices() && $order->canInvoice()) {
            ...
            if ($invoice->getTotalQty() > 0) {
                ...
                $transaction->save();
                $invoice->addComment(Mage::helper('epdq')->__('Automatic invoice.'), false);
    ...
    {
        $this->restore();
        $message = Mage::helper('epdq')->__('Payment failed. Please try again.');
    ...
    {
        $this->getCheckout()->setQuoteId($this->getCheckout()->getePDQQuoteId(true));
        $this->getCheckout()->setLastOrderId($this->getCheckout()->getePDQOrderId(true));
    }
